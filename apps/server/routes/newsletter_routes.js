...
export function registerNewsletterRoutes(fastify) {
  ...
  fastify.post(
    "/newsletter/check-subscriber",
    {
      schema: {
        tags: ["Newsletter"],
        summary: "Check if email is already subscribed",
        description:
          "Check if an email address is already subscribed to the newsletter",
        body: {
          type: "object",
          required: ["email"],
          properties: {
    ...
    async (request, reply) => {
      try {
        ...
        // If service is disabled, return a specific error
        if (result.disabled) {
  ...
  fastify.post(
    "/newsletter/subscribe-beta",
    {
      schema: {
        tags: ["Newsletter"],
        summary: "Subscribe email to newsletter with beta tag",
        description:
          "Add an email address to the newsletter subscription list with beta tag",
        body: {
          type: "object",
          required: ["email"],
          properties: {
    ...
    async (request, reply) => {
      try {
        ...
        const result = await convertKit.addBetaSubscriber(
          email,
          firstName,
          lastName
        );
  ...
  fastify.post(
      ...
      schema: {
        ...
        response: {
          200: {
            ...
            description:
              "Returns success message for newsletter confirmation email sent",
          },
          400: {
            type: "object",
            properties: {
    ...
    async (request, reply) => {
      try {
        ...
        console.log("Newsletter confirmation email request received:", {
          email,
        });
        ...
        if (!serviceConfig.isEmailEnabled()) {
          return reply.status(503).send({
            error: "Email service is currently unavailable",
            details: serviceConfig.getEmailConfig().reason,
          });
        }
        ...
        try {
          console.log(
            "Attempting to send newsletter confirmation email to:",
            email
          );
          ...
          console.log(
            "- MAIL_FROM_NAME:",
            process.env.MAIL_FROM_NAME || "Bumper Vehicles"
          );
          ...
          console.log(
            "Newsletter confirmation email sent successfully to:",
            email
          );
        } catch (emailError) {
          console.error(
        ...
        return reply.send({
          success: true,
          message:
            "Confirmation email sent! Please check your inbox to complete your subscription.",
        });
      } catch (error) {
