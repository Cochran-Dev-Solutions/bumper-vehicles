  ...
  fastify.post(
    "/payment/capture",
    {
      schema: {
        ...
        description:
          "Webhook endpoint to capture a PayPal payment after approval",
        body: {
          type: "object",
          required: ["orderId"],
          properties: {
  ...
  fastify.get(
    "/payment/success",
    {
      schema: {
        ...
        description:
          "Callback endpoint for successful PayPal payments. Creates beta user account and sends credentials.",
        querystring: {
          type: "object",
          properties: {
            token: { type: "string" },
    ...
    async (request, reply) => {
      try {
        ...
        if (!paymentDetails.success) {
          ...
          return reply
            .status(500)
            .send({ error: "Failed to get payment details" });
        }
        ...
        console.log("Creating beta user account for:", {
          email,
          firstName,
          lastName,
        });
        ...
        if (existingUserCheck.exists) {
          ...
          if (existingUser.success) {
            ...
            try {
              ...
              console.log(
                "Beta credentials email sent successfully for existing user:",
                email
              );
            } catch (emailError) {
              console.error(
                "Failed to send beta credentials email:",
                emailError.message
              );
            }
            ...
            const baseUrl = isProduction
              ? process.env.PROD_LANDING_PAGE_HOST_URL ||
                "https://bumpervehicles.com"
              : process.env.LANDING_PAGE_HOST_URL || "http://localhost:5174";
            const credentialsUrl = `${baseUrl}/beta-credentials?username=${encodeURIComponent(
              existingUser.user.username
            )}&password=Your existing password&email=${encodeURIComponent(
              email
            )}`;
            console.log(
              "Redirecting to beta credentials page:",
              credentialsUrl
            );
        ...
        const betaUserResult = await betaUserDal.createBetaUser(
          email,
          firstName,
          lastName
        );
        ...
        if (!betaUserResult.success) {
          ...
          return reply
            .status(500)
            .send({ error: "Failed to create beta user account" });
        }
        ...
        try {
          await sendBetaCredentialsEmail(
            email,
            betaUserResult.username,
            betaUserResult.password
          );
          console.log(
            "Beta credentials email sent successfully for new user:",
            email
          );
        } catch (emailError) {
          console.error(
  ...
  fastify.get(
    "/payment/cancel",
    {
      schema: {
        ...
        description:
          "Callback endpoint for cancelled PayPal payments. Redirects to landing page with cancel message.",
      },
    },
    async (request, reply) => {
