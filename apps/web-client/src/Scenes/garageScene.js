...
function drawPlayerInfoPanel(p) {
  ...
  if (testPlayerData.isFriend) {
    ...
    p.text(
      "✓ Friends",
      friendButtonX + friendButtonWidth / 2,
      friendButtonY + friendButtonHeight / 2
    );
  } else {
...
const garageScene = {
  ...
  init: async function () {
    ...
    landing_bay_height =
      (this.p.height - garage_ceiling_height - garage_floor_height) / 2;

    // create map character
    this.garageCharacter = new GarageCharacter(
      this.p,
      this.p.width / 2,
      this.p.height / 2 - 45
    );
  ...
  display: function () {
    ...
    this.p.push();
    this.p.translate(
      this.p.width - p5Images["mystery_box"].width / 3,
      this.p.height - p5Images["mystery_box"].height / 3
    );
    this.p.scale(2 / 3);
    this.p.image(
