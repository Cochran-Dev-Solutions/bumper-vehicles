...
export default class Database {
  ...
  async register_user(config, cb) {
    ...
    try {
      ...
      await client.query(
        "INSERT INTO unverified_users (username, email, password, verification_code, expires_at) VALUES ($1, $2, $3, $4, $5)",
  ...
  async verify_user(config, cb) {
    const query = "SELECT * FROM unverified_users WHERE verification_code = $1 AND expires_at > NOW()";
  ...
  async resend_verification_email(config, cb) {
    ...
    if (rows.length > 0) {
      ...
      await this.db.query(
        "UPDATE unverified_users SET verification_code = $1, expires_at = $2 WHERE email = $3",
